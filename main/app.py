...
def create_app():
    ...
    def create_new_parking():
        ...
        new_parking_place = Parking(address=address,
    ...
    def moving_the_car_to_the_parking_place():
        ...
        credit_card_check = db.session.query(Client.credit_card).where(Client.id == client_id).one_or_none()

        if credit_card_check is None or credit_card_check[0] == '':
            return f'Client with id={client_id} does not have any credit card.', 400

        parking_place_open = db.session.query(Parking.opened).where(Parking.id == parking_id).one_or_none()

        count_available_places = db.session.query(
            Parking.count_available_places).where(Parking.id == parking_id).one()[0]

        if parking_place_open is None or parking_place_open[0] is False or (count_available_places - 1) < 0:
        ...
        new_happy_car = ClientParking(client_id=client_id, parking_id=parking_id, time_in=time_in, time_out=time_out)
        db.session.add(new_happy_car)
        db.session.commit()

        new_happy_car_log = ParkingLog(client_id=client_id, parking_id=parking_id,
    ...
    def delete_client_from_parking():
        ...
        record_exists = bool(ClientParking.query.filter(
            and_(ClientParking.client_id == client_id, ClientParking.parking_id == parking_id)).first())

        if not record_exists:
            return f'Combination client_id={client_id}, and parking_id={parking_id} is not exist.', 404

        db.session.query(ParkingLog).filter(
            and_(ParkingLog.client_id == client_id, ParkingLog.parking_id == parking_id)).update(
            {ParkingLog.time_out: datetime.utcnow()})

        db.session.query(ClientParking).filter(
            and_(ClientParking.client_id == client_id, ClientParking.parking_id == parking_id)).delete()
